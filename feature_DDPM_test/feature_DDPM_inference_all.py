...
def p_sample_loop(model, latent, shape):
    ...
    t = torch.full((latent.size(0),), 200 - 1, device=device, dtype=torch.long)
    noise = torch.randn_like(latent).cuda()
    x_t = q_sample(latent, t, noise)

    for t_index in reversed(range(200)):
...
for main_folder in main_folders:
    ...
    for subfolder in subfolders:
        ...
        for idx in range(len(dataset)):
            ...
            for i in range(5):
                generated_vector = p_sample_loop(model, latent, (1, 1, 768)).squeeze(0)

                # 确保生成的向量不需要梯度
                generated_vector = generated_vector.detach()

                # 创建对应的推理输出目录
                inference_step_dir = os.path.join(inference_sub_dir, f'inference_step200_{i}')
